...
int vals[] = {0, 0, 0};
int avgVals[] = {0, 0, 0};
int colordelay = 20;       // Delay (in ms) between readings to allow phototransistor to come to steady state 
int numScans = 6;          // Number of scans to average together

// Calibration Variables (CALIBRATED AT HOME AT 1 AM)
const int tol = 40;                       // RGB Color Tolerance
const int redCal[] = {399, 232, 189};     // Red M&M
const int greenCal[] = {272, 383, 221};   // Green M&M
const int blueCal[] = {253, 301, 413};    // Blue M&M
const int yellowCal[] = {556, 434, 189};  // Yellow M&M
const int orangeCal[] = {506, 239, 202};  // Orange M&M
const int brownCal[] = {271, 243, 214};   // Brown M&M
const int emptyCal[] = {299, 309, 385};   // Empty slot
const int plateCal[] = {570, 583, 540};   // Misaligned plate
...
char colorScan(void) {
  // Runs color scan and outputs averaged RGB values
  ...
  for (int i = 0; i < numScans; i++) {
    ...
    // Set LED color to Green and read
    ...
    // Set LED color to Blue and read
    digitalWrite(COLOR_RED, LOW);
    digitalWrite(COLOR_GREEN, LOW);
...
char parseColor(void) {
  // Function to parse the color based on RGB readings
  ...
  for (int i = 0; i < 3; i++) { // Iterates through each scan value (RGB) 
